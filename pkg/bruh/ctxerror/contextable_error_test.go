...
func TestContextableError(t *testing.T) {
	...
	ctx := GetContext(err)
	assert.Equal(len(ctx), 2, "New returned error with context %+v, expected %+v", ctx, map[string]any{key1: value1, key2: value2})
	...
	ctx = GetContext(err)
	assert.Equal(len(ctx), 2, "NewSkip returned error with context %+v, expected %+v", ctx, map[string]any{key1: value1, key2: value2})
	...
	ctx = GetContext(err)
	assert.Equal(len(ctx), 2, "Wrap returned error with context %+v, expected %+v", ctx, map[string]any{key1: value1, key2: value2})
	...
	ctx = GetContext(err)
	assert.Equal(len(ctx), 2, "WrapSkip returned error with context %+v, expected %+v", ctx, map[string]any{key1: value1, key2: value2})
}
...
func TestContextableError_Modifiers(t *testing.T) {
	...
	require.NotNil(err, "New returned nil error")
	assert.Equal(err.Context(), map[string]any{key1: value2, key2: value2}, "Add did not overwrite the expected context")

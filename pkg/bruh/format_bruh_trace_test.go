...
func TestFormatBruh(t *testing.T) {
	...
	assertBruh := func(name string, err error, exp string) {
		t.Run(name, func(t *testing.T) {
			result := bruhTraceReplacePath(bruh.StringFormat(err, bruh.BruhFormatter))
			if result != exp {
				t.Fatalf("expected:\n|%s|\n\ngot:\n|%s|", exp, result)
			}
			result = bruhTraceReplacePath(bruh.String(err))
			if result != exp {
				t.Fatalf("expected:\n|%s|\n\ngot:\n|%s|", exp, result)
			}
			if err != nil && name != "ExternallyWrapped" {
				result = bruhTraceReplacePath(fmt.Sprintf("%+v", err))
				if result != exp {
					t.Fatalf("expected:\n|%s|\n\ngot:\n|%s|", exp, result)
				}
			}
	...
	assertBruh("Nil", nil, "")
	assertBruh("SingleRoot", singleRootError, `root error
	...
	assertBruh("EmptyMessage", emptyMessageError, `<no message>
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("Wrapped", wrappedError, `wrapped 3: wrapped 2: wrapped 1: root error
    at github.com/aisbergg/go-bruh/pkg/bruh_test.singleRootError (/pkg/bruh/format_test.go:23)
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("WrappedEmptyMessage", wrappedEmptyMessageError, `<no message>
    at github.com/aisbergg/go-bruh/pkg/bruh_test.emptyMessageError (/pkg/bruh/format_test.go:28)
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("External", externalError, `external error`)
	assertBruh("WrappedExternal", wrappedExternalError, `wrapped 1: external error
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("ExternallyWrapped", externallyWrappedError, `external error: root error
    at github.com/aisbergg/go-bruh/pkg/bruh_test.singleRootError (/pkg/bruh/format_test.go:23)
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("WrappedExternalInterleaved", wrappedExternalInterleavedError, `wrapped: external error: root error
    at github.com/aisbergg/go-bruh/pkg/bruh_test.singleRootError (/pkg/bruh/format_test.go:23)
    at github.com/aisbergg/go-bruh/pkg/bruh_test.externallyWrappedError (/pkg/bruh/format_test.go:70)
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("ExternallyWrappedNil", externallyWrappedNilError, `wrapped: external error: %!w(<nil>)
    ...
    at testing.tRunner (/testing/testing.go:1234)`)
	assertBruh("WrappedGlobal", wrappedGlobalError, `wrapped: globally wrapped: root error

...
func TestFormatPythonTraceback(t *testing.T) {
	...
	assertPythonTraceback := func(name string, err error, exp string) {
		t.Run(name, func(t *testing.T) {
			result := pythonTracebackReplacePath(bruh.StringFormat(err, bruh.PythonTracebackFormatter))
			if result != exp {
				t.Errorf("expected:\n|%s|\n\ngot:\n|%s|", exp, result)
			}
		})
	...
	assertPythonTraceback("Nil", nil, "")
	assertPythonTraceback("SingleRoot", singleRootError, `Traceback (most recent call last):
...
*bruh.Err: root error`)
	assertPythonTraceback("EmptyMessage", emptyMessageError, `Traceback (most recent call last):
  File "/testing/testing.go", line 1234, in testing.tRunner
...
*bruh.Err`)
	assertPythonTraceback("Wrapped", wrappedError, `Traceback (most recent call last):
...
*bruh.Err: wrapped 3`)
	assertPythonTraceback("WrappedEmptyMessage", wrappedEmptyMessageError, `Traceback (most recent call last):
...
*bruh.Err`)
	assertPythonTraceback("External", externalError, `*errors.errorString: external error`)
	assertPythonTraceback("WrappedExternal", wrappedExternalError, `*errors.errorString: external error
...
*bruh.Err: wrapped 1`)
	assertPythonTraceback("ExternallyWrapped", externallyWrappedError, `Traceback (most recent call last):
  File "/testing/testing.go", line 1234, in testing.tRunner
...
*bruh.Err: wrapped`,
	...
	assertPythonTraceback("ExternallyWrappedNil", externallyWrappedNilError, `*fmt.wrapError: external error: %!w(<nil>)
...
*bruh.Err: wrapped`)
	assertPythonTraceback("WrappedGlobal", wrappedGlobalError, `*bruh.Err: root error
